...
## Overview
Add a full dark mode for the app with a theme toggle available on the Auth Landing screen and on the Home dashboard. The theme preference is persisted and restored on launch. The design keeps the existing coral/teal/amber accents while adjusting surfaces, borders, text, shadows, and backgrounds for dark readability.

## Goals
- Provide light and dark themes with complete coverage across screens and shared components.
- Allow users to toggle the theme from Auth Landing and Home (floating top-right button).
...
## UX / Display Behavior
- Theme toggle button appears at the top-right of:
  - Auth Landing screen
  - Home screen
...
- New `ThemeToggle` component in `src/components/ThemeToggle.tsx`:
  - Floating, circular button using theme tokens and safe-area insets.
...
Manual tests:
- Toggle from Auth Landing and Home screens; verify the icon flips.
